...
export default async function CoursePage({params}:{params:Promise<{courseSlug:string}>}){
 ...
 <div className="topbar"><div><div className="eyebrow">{course.level}</div><h1>{course.title}</h1><p className="muted">{course.description}</p></div><span className="badge">{pct}% concluído</span></div><div className="progress section"><span style={{width:`${pct}%`}}/></div>
 ...
 {(m.quizzes||[]).filter((q:any)=>!q.lesson_id).map((q:any)=><Link href={`/teste/${q.id}`} className="lesson-row quiz-row" key={q.id}><span className="status-dot quiz-dot"></span><div><strong>{q.title}</strong><div className="muted small">Teste de avaliação do módulo</div></div><span className="row-arrow">→</span></Link>)}
