...
export default async function LessonPage({params}:{params:Promise<{lessonId:string}>}){
 ...
 <article className="reading"><div className="lesson-stage"><div><div className="eyebrow">Módulo {mod?.position} · Aula {moduleIdx+1} de {sameModule.length} · {lesson.duration_minutes||8} min</div><h1>{lesson.title}</h1></div><div className="lesson-stage-progress"><span>{coursePct}%</span><div className="mini-progress"><i style={{width:`${coursePct}%`}}/></div></div></div><p className="lead">{content.intro}</p>{content.sections.map((s:any,i:number)=><section className="reading-section" key={i}><h2>{s.title}</h2><p>{s.body}</p></section>)}{content.case_body&&<section className="case-box"><div className="eyebrow">{content.case_title||'Caso prático'}</div><p>{content.case_body}</p></section>}{content.takeaway&&<section className="takeaway"><strong>Ideia principal</strong><p>{content.takeaway}</p></section>}<LessonInteractive lessonTitle={lesson.title} courseSlug={course?.slug}/>{lesson.legal_sensitive&&<section className="legal-source"><strong>Conteúdo legal/documental verificado</strong><div>Última verificação: {lesson.verified_at||'—'}</div>{lesson.source_url&&<a href={lesson.source_url} target="_blank" rel="noreferrer">Consultar fonte oficial ↗</a>}</section>}
