...
export default async function TestPage({params}:{params:Promise<{quizId:string}>}){
 ...
 const {data:q}=await supabase.from('quizzes').select('id,title,lesson_id,module_id,lessons(id,title,module_id,modules(id,title,position,courses(id,title,slug))),modules(id,title,position,courses(id,title,slug))').eq('id',id).single();if(!q)notFound()
