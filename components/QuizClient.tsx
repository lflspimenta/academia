...
export default function QuizClient({quizId}:{quizId:number}){
 ...
 useEffect(()=>{(async()=>{
   setLoading(true);setError('');setResult(null);setSelected({});setReviewingPrevious(false)
   const [{data:q,error:qError},{data:last,error:lastError}]=await Promise.all([
     supabase.rpc('get_quiz_safe',{p_quiz_id:quizId}),
     supabase.rpc('get_latest_quiz_attempt_review',{p_quiz_id:quizId})
   ])
   if(qError){setError('Não foi possível carregar o teste. Tente novamente.');setLoading(false);return}
   setQuiz(q as Quiz)
   if(!lastError&&last){
     setResult(last)
     const previous:Record<number,number>={}
     for(const a of (last.answers||[])){if(a?.question_id&&a?.answer_id)previous[Number(a.question_id)]=Number(a.answer_id)}
     setSelected(previous)
     setReviewingPrevious(true)
   }
   setLoading(false)
 })()},[quizId])
 async function submit(){if(!quiz)return;setError('');const missing=quiz.questions.some(q=>!selected[q.id]);if(missing){setError('Responda a todas as perguntas antes de submeter.');return}setLoading(true);const payload=quiz.questions.map(q=>({question_id:q.id,answer_id:selected[q.id]}));const {data,error}=await supabase.rpc('submit_quiz',{p_quiz_id:quizId,p_answers:payload});if(error)setError('Não foi possível submeter o teste. Tente novamente.');else {setResult(data);setReviewingPrevious(false)}setLoading(false)}
